...
const char* BANK_CODE_NAME = "triceratops_factory_banks";

static std::vector<BankInfo_Converter> s_banks;

static bool is_triceratops_bank_file(std::string filename)
{
...
static void scan_preset_bank(const std::string dir_path, const std::string file_name)
{
    ...
    if (!is_triceratops_bank_file(file_path.c_str()))
        return;
...
static void scan_preset_banks(const std::string scan_path)
{
    ...
    while ((entry = readdir(dir))) {
        // clang-format off
        if (std::string(entry->d_name) != ".directory"
            && std::string(entry->d_name) != "."
            && std::string(entry->d_name) != ".."
            && std::string(entry->d_name) != "manifest.ttl") { // If sub directory, "."/".." or manifest.ttl detected, ignore
            // clang-format on
    ...
    for (auto& filename : filenames)
        scan_preset_bank(scan_path, filename);
}
...
bool start_convert(const std::string bank_directory, const std::string output_dir)
{
    ...
    fprintf(out_cpp, "#include <presets.hpp>\n");
    ...
    for (auto iter = s_banks.begin(); iter != s_banks.end(); iter++) {
        if (iter->success) {
            ...
            fprintf(out_cpp, "// WARNING: Bank file \"%s\" failed to read\n", iter->name.c_str());
            fprintf(out_cpp, "//          (%s)\n\n", iter->file_path.c_str());
        }
    }
